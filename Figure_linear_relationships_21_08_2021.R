# This script will produce plots of the average linear relationships between
...
# and Sea Surface Temperature, inferred from 
# ARCTIC REANALYSIS PHYS 002 003 available on https://marine.copernicus.eu/
# [chl-a] and Windy events, inferred from WIND GLO WIND L3 REP OBSERVATIONS 012 005
# available on https://marine.copernicus.eu/
# [chl-a] and and changes in current velocities, inferred from 
# ARCTIC REANALYSIS PHYS 002 003 available on https://marine.copernicus.eu/
# and SST and Windy events.
...
for(a in 1:(length(lat_steps)-1)){ # for each latitude step 
	for(b in 1:(length(lon_steps)-1)){ # for each longitude step
		...
		biomass<-dataset[which(dataset$lat==lat_steps[a] & dataset$lon==lon_steps[b]),]$chla # fetch [chl-a] data
			if( length(which(is.na(temp)==F))>3 & sd(temp,na.rm=T)>0 & length(which(is.na(biomass)==F))>3 & sd(biomass,na.rm=T)>0){ # Check that the data is good
			chosen_row<-which(trends$lat==lat_steps[a] & trends$lon==lon_steps[b])
			df1<-as.data.frame(cbind(biomass,temp,years))
			df1<-df1[complete.cases(df1),]
			mod1<-gls( biomass ~ temp ,data=df1) # Fit a standard GLS model
			tryCatch( mod2<-gls( biomass ~ temp  ,correlation=corAR1(form=~years),data= df1) ,error=function(e) print('test') )
...
# 'Chlor_3' is a reprojected summary of the linear relationships from the preferred model fits
...
labs(fill=expression(paste(Delta, italic('B'), italic(' SST')^-1)) )+
...
# type 'B_v_SST' into the terminal to view the plot
...
for(a in 1:(length(lat_steps)-1)){
	for(b in 1:(length(lon_steps)-1)){
		...
		biomass<-dataset[which(dataset$lat==lat_steps[a] & dataset$lon==lon_steps[b]),]$chla
			if( length(which(is.na(wind)==F))>3 & sd(wind,na.rm=T)>0 & length(which(is.na(biomass)==F))>3 & sd(biomass,na.rm=T)>0){
			chosen_row<-which(trends$lat==lat_steps[a] & trends$lon==lon_steps[b])
			df1<-as.data.frame(cbind(biomass,wind,years))
			df1<-df1[complete.cases(df1),]
			mod1<-gls( biomass ~ wind ,data=df1)
			tryCatch( mod2<-gls( biomass ~ wind  ,correlation=corAR1(form=~years),data= df1) ,error=function(e) print('test') )
			pval<-1
			if(exists('mod2')){
...
aes(x=lon,y=lat), label=c(expression(paste('0'^o,'E')),expression(paste('30'^o,'E')),expression(paste('60'^o,'E')),expression(paste('75'^o,'N')),expression(paste('80'^o,'N'))),
angle=c(-30,0,30,30,30))+
labs(fill=expression(paste(Delta, italic('B'), italic(' W')^-1)) )+
theme(axis.line=element_blank(),
      axis.text.x=element_blank(),
      axis.text.y=element_blank(),
      axis.ticks=element_blank(),
...
for(a in 1:(length(lat_steps)-1)){
	for(b in 1:(length(lon_steps)-1)){
		...
		biomass<-dataset[which(dataset$lat==lat_steps[a] & dataset$lon==lon_steps[b]),]$chla
			if( length(which(is.na(currents)==F))>3 & sd(currents,na.rm=T)>0 & length(which(is.na(biomass)==F))>3 & sd(biomass,na.rm=T)>0){
			chosen_row<-which(trends$lat==lat_steps[a] & trends$lon==lon_steps[b])
			df1<-as.data.frame(cbind(biomass,currents,years))
			df1<-df1[complete.cases(df1),]
			mod1<-gls( biomass ~ currents ,data=df1)
			tryCatch( mod2<-gls( biomass ~ currents  ,correlation=corAR1(form=~years),data= df1) ,error=function(e) print('test') )
			pval<-1
			if(exists('mod2')){
					pval<-anova(mod1,mod2)$p[2]
			} 
...
aes(x=lon,y=lat), label=c(expression(paste('0'^o,'E')),expression(paste('30'^o,'E')),expression(paste('60'^o,'E')),expression(paste('75'^o,'N')),expression(paste('80'^o,'N'))),
angle=c(-30,0,30,30,30))+
labs(fill=expression(paste(Delta, italic('B'), italic(' C')^-1)) )+
theme(axis.line=element_blank(),
      axis.text.x=element_blank(),
      axis.text.y=element_blank(),
      axis.ticks=element_blank(),
...
aes(x=lon,y=lat), label=c(expression(paste('0'^o,'E')),expression(paste('30'^o,'E')),expression(paste('60'^o,'E')),expression(paste('75'^o,'N')),expression(paste('80'^o,'N'))),
angle=c(-30,0,30,30,30))+
labs(fill=expression(paste(Delta, italic('SST'), italic(' W')^-1)) )+
theme(axis.line=element_blank(),
      axis.text.x=element_blank(),
      axis.text.y=element_blank(),
      axis.ticks=element_blank(),
...
ggarrange(B_v_SST,B_v_W,B_v_C,SST_v_W,ncol=2,nrow=2,labels=c('a','b','c','d'),label.x=0.175,label.y=0.8,font.label=list(size=24),
align='hv')
#ggsave(filename='Linear_relationships_13_07_2021.pdf',dpi=300)
